...
def var_filter(
    ...
    na_perc = (
        dt[x].apply(nan_rate).reset_index(name="missing_rate").rename(columns={"index": "variable"})
    )
    ...
    if return_rm_reason:
        dt_var_rm = dt_var_selector.query(
            ...
            info_value=lambda x: [
                "info_value<{}".format(iv_limit) if i else np.nan for i in (x.info_value < iv_limit)
            ],
