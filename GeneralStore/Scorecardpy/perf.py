...
def eva_dfkslift(df, groupnum=None):
    ...
    df_kslift = (
        ...
        .assign(
            ...
            lift=lambda x: (np.cumsum(x.bad) / np.cumsum(x.good + x.bad))
            / (sum(x.bad) / sum(x.good + x.bad)),
...
def eva_dfrocpr(df):
    ...
    dfrocpr = (
        ...
        .agg([n0, n1, len])
        .reset_index()
        .rename(columns={"n0": "countN", "n1": "countP", "len": "countpred"})
        .assign(FN=lambda x: np.cumsum(x.countP), TN=lambda x: np.cumsum(x.countN))
        .assign(TP=lambda x: sum(x.countP) - x.FN, FP=lambda x: sum(x.countN) - x.TN)
        .assign(
        ...
        .assign(F1=lambda x: 2 * x.precision * x.recall / (x.precision + x.recall))
    )
    return dfrocpr
...
def eva_proc(dfrocpr, title):
    dfrocpr = pd.concat(
        [dfrocpr[["FPR", "TPR"]], pd.DataFrame({"FPR": [0, 1], "TPR": [0, 1]})], ignore_index=True
    ).sort_values(["FPR", "TPR"])
    auc = (
        dfrocpr.sort_values(["FPR", "TPR"])
        .assign(TPR_lag=lambda x: x["TPR"].shift(1), FPR_lag=lambda x: x["FPR"].shift(1))
        .assign(auc=lambda x: (x.TPR + x.TPR_lag) * (x.FPR - x.FPR_lag) / 2)["auc"]
        .sum()
    )
    ###### plot ######
    # fig, ax = plt.subplots()
    ...
    # set xylabel
    plt.gca().set(
        title=title + "ROC", xlabel="FPR", ylabel="TPR", xlim=[0, 1], ylim=[0, 1], aspect="equal"
    )
    # text
...
def perf_psi(
    ...
    if not isinstance(score, dict) and len(score) != 2:
        ...
        if set(score_columns[0]) != set(score_columns[1]):
            raise Exception(
                "Incorrect inputs; the column names of two dataframes in score should be the same."
            )
    ## label
    if label is not None:
        if not isinstance(label, dict) and len(label) != 2:
            ...
            if set(score.keys()) != set(label.keys()):
                raise Exception(
                    "Incorrect inputs; the keys of score and label should be the same. "
                )
            for i in label.keys():
                if isinstance(label[i], pd.DataFrame):
                    if len(label[i].columns) == 1:
                        ...
                        raise Exception(
                            "Incorrect inputs; the number of columns in label should be 1."
                        )
    ...
    def psi(dat):
        dt_bae = (
            ...
            .size()
            .reset_index(name="N")
            .pivot_table(values="N", index="bin", columns="ae")
            .fillna(0.9)
            .agg(lambda x: x / sum(x))
        )
        dt_bae.columns = ["A", "E"]
        psi_dt = (
            dt_bae.assign(AE=lambda x: x.A - x.E, logAE=lambda x: np.log(x.A / x.E))
            .assign(bin_PSI=lambda x: x.AE * x.logAE)["bin_PSI"]
            .sum()
        )
        return psi_dt
    ...
    for sn in score_names:
        ...
        if len(dt_sl[sn].unique()) > 10:
            ...
            brkp = np.unique(
                [np.floor(min(dt_sl[sn]) / x_tick_break) * x_tick_break]
                + list(np.arange(x_limits[0], x_limits[1], x_tick_break))
                + [np.ceil(max(dt_sl[sn]) / x_tick_break) * x_tick_break]
            )
            # cut
            labels = [
                "[{},{})".format(int(brkp[i]), int(brkp[i + 1])) for i in range(len(brkp) - 1)
            ]
        ...
        distr_prob = (
            dat.groupby(["ae", "bin"])["y"]
            .agg([good, bad])
            .assign(N=lambda x: x.good + x.bad, badprob=lambda x: x.bad / (x.good + x.bad))
            .reset_index()
        )
        distr_prob.loc[:, "distr"] = distr_prob.groupby("ae")["N"].transform(lambda x: x / sum(x))
        # pivot table
        distr_prob = distr_prob.pivot_table(
            values=["N", "badprob", "distr"], index="bin", columns="ae"
        )
    ...
    # return rt
    rt["psi"] = (
        pd.concat(rt_psi).reset_index().rename(columns={"level_0": "variable"})[["variable", "PSI"]]
    )
    rt["pic"] = rt_pic
    if return_distr_dat:
        rt["dat"] = rt_dat
