...
def rep_blank_na(dat):  # cant replace blank string in categorical value with nan
    ...
    blank_cols = [
        col
        for col in list(dat)
        if dat[col].astype(str).str.findall(r"^\s*$").apply(lambda x: 0 if len(x) == 0 else 1).sum()
        > 0
    ]
    if len(blank_cols) > 0:
        warnings.warn(
    ...
    if len(cols_num) > 0:
        cols_inf = [col for col in cols_num if any(dat[col] == np.inf) | any(dat[col] == -np.inf)]
        if len(cols_inf) > 0:
            warnings.warn(
...
def check_y(dat, y, positive):
    ...
    if len(unique_y) == 2:
        ...
        if True in [bool(re.search(positive, str(v))) for v in unique_y]:
            ...
            raise Exception(
                'Incorrect inputs; the positive value in "{}" is not specified'.format(y)
            )
    else:
        raise Exception(
            "Incorrect inputs; the length of unique values in y column '{}' != 2.".format(y)
        )
