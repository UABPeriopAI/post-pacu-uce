...
def scorecard(
    bins, model, xcolumns, points0=600, odds0=1 / 19, pdo=50, basepoints_eq0=False, digits=0
):
    """
    ...
    # coefficients
    coef_df = pd.Series(model.coef_[0], index=np.array(xs)).loc[
        lambda x: x != 0
    ...
    if basepoints_eq0:
        card["basepoints"] = pd.DataFrame(
            {"variable": "basepoints", "bin": np.nan, "points": 0}, index=np.arange(1)
        )
        for i in coef_df.index:
            card[i] = bins.loc[bins["variable"] == i, ["variable", "bin", "woe"]].assign(
                points=lambda x: round(-b * x["woe"] * coef_df[i] + basepoints / len_x),
                ndigits=digits,
            )[["variable", "bin", "points"]]
    else:
        card["basepoints"] = pd.DataFrame(
            {"variable": "basepoints", "bin": np.nan, "points": round(basepoints, ndigits=digits)},
            index=np.arange(1),
        )
        for i in coef_df.index:
            card[i] = bins.loc[bins["variable"] == i, ["variable", "bin", "woe"]].assign(
                points=lambda x: round(-b * x["woe"] * coef_df[i]), ndigits=digits
            )[["variable", "bin", "points"]]
    return card
...
def scorecard_ply(
    ...
    # set basepoints
    card_basepoints = (
        list(card_df.loc[card_df["variable"] == "basepoints", "points"])[0]
        if "basepoints" in card_df["variable"].unique()
        else 0
    )
    # total score
