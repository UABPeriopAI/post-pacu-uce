...
def dtm_binning_sv(dtm, breaks, spl_val):
    ...
    if spl_val is not None:
        ...
        binning_sv = (
            ...
            .groupby(["variable", "rowid", "bin_chr"])
...
def check_empty_bins(dtm, binning):
    ...
    if "nan" in bin_list:
        bin_list.remove("nan")
    binleft = set([re.match(r"\[(.+),(.+)\)", i).group(1) for i in bin_list]).difference(
        set(["-inf", "inf"])
    )
    binright = set([re.match(r"\[(.+),(.+)\)", i).group(2) for i in bin_list]).difference(
        set(["-inf", "inf"])
    )
    if binleft != binright:
...
def woebin2_breaks(dtm, breaks, spl_val):
    ...
    if is_numeric_dtype(dtm["value"]):
        ...
        bstbrks = (
            ["-inf"]
            + list(
                set(bk_df.value.tolist()).difference(set([np.nan, "-inf", "inf", "Inf", "-Inf"]))
            )
            + ["inf"]
        )
        ...
        binning = (
            pd.merge(
                binning.assign(
                    value=lambda x: [
                        float(re.search(r"^\[(.*),(.*)\)", i).group(2)) if i != "nan" else np.nan
        ...
        if bk_df["value"].isnull().any():
            binning = (
                binning.assign(bin=lambda x: [i if i != "nan" else "missing" for i in x["bin"]])
                .fillna("missing")
...
def woebin2_init_bin(dtm, init_count_distr, breaks, spl_val):
    ...
    while len(init_bin.query("(good==0) or (bad==0)")) > 0:
        ...
        init_bin = init_bin.assign(brkp2=lambda x: x["brkp"].shift(shift_period)).assign(
            brkp=lambda x: np.where(x["brkp"] == rm_brkp["brkp"], x["brkp2"], x["brkp"])
        )
        # groupby brkp
        init_bin = (
            init_bin.groupby("brkp")
            .agg(
                {
    ...
    if is_numeric_dtype(dtm["value"]):
        init_bin = init_bin.assign(
            bin=lambda x: [
                re.sub(r"(?<=,).+%,%.+,", "", i) if ("%,%" in i) else i for i in x["bin"]
            ]
...
def woebin2_tree_add_1brkp(dtm, initial_binning, count_distr_limit, bestbreaks=None):
    ...
    def total_iv_all_breaks(initial_binning, bestbreaks, dtm_rows):
        ...
        total_iv_all_brks["count_distr"] = (
            total_iv_all_brks.groupby(["variable", "bstbin"])["count"]
    ...
    def binning_add_1bst(initial_binning, bestbreaks):
        ...
        if is_numeric_dtype(dtm["value"]):
            binning_1bst_brk = (
                binning_1bst_brk.groupby(["variable", "bstbin"])
                .agg({"good": sum, "bad": sum})
                .reset_index()
                .assign(bin=lambda x: x["bstbin"])[["bstbin", "variable", "bin", "good", "bad"]]
            )
        else:
            binning_1bst_brk = (
                binning_1bst_brk.groupby(["variable", "bstbin"])
        ...
        binning_1bst_brk["bstbrkp"] = [
            float(re.match("^\[(.*),.+", i).group(1)) for i in binning_1bst_brk["bstbin"]
        ]
        # return
...
def woebin2_chimerge(
    ...
    def add_chisq(initial_binning):
        ...
        ###
        chisq_df["a_lag"] = chisq_df.groupby("goodbad")["a"].apply(
            lambda x: x.shift(1)
        )  # .reset_index(drop=True)
        chisq_df["a_rowsum"] = chisq_df.groupby("brkp")["a"].transform(
            lambda x: sum(x)
        )  # .reset_index(drop=True)
        ...
        chisq_df = (
            ...
            .assign(ae=lambda df: (df.a - df.e) ** 2 / df.e + (df.a_lag - df.e_lag) ** 2 / df.e_lag)
            .groupby("brkp")
        ...
        # return
        return pd.merge(
            initial_binning.assign(count=lambda x: x["good"] + x["bad"]), chisq_df, how="left"
        )
    ...
    while (
        ...
        if bin_chisq_min < chisq_limit:
            rm_brkp = (
                binning_chisq.assign(merge_tolead=False).sort_values(by=["chisq", "count"]).iloc[0,]
            )
        ...
        # set brkp to lead's or lag's
        shift_period = -1 if rm_brkp["merge_tolead"] else 1
        binning_chisq = binning_chisq.assign(brkp2=lambda x: x["brkp"].shift(shift_period)).assign(
            brkp=lambda x: np.where(x["brkp"] == rm_brkp["brkp"], x["brkp2"], x["brkp"])
        )
        # groupby brkp
        binning_chisq = (
            binning_chisq.groupby("brkp")
            .agg(
                {
                    "variable": lambda x: np.unique(x),
                    "bin": lambda x: "%,%".join(x),
...
def bins_to_breaks(bins, dt, to_string=False, save_string=None):
    ...
    vars_class = pd.DataFrame(
        {"variable": xs_all, "not_numeric": [not is_numeric_dtype(dt[i]) for i in xs_all]}
    )
    ...
    if no_cores == 1:
        ...
        for i in np.arange(xs_len):
            ...
            if print_step > 0 and bool((i + 1) % print_step):
                print(
                    ("{:" + str(len(str(xs_len))) + ".0f}/{} {}").format(i, xs_len, x_i), flush=True
                )
        ...
        args = zip(
            ...
            [
                special_values[i]
                if (special_values is not None) and (i in list(special_values.keys()))
                else None
                for i in xs
            ],
    ...
    if runingtime >= 10 and print_info:
        ...
        print(
            "Binning on {} rows and {} columns in {}".format(
                dt.shape[0], dt.shape[1], time.strftime("%H:%M:%S", time.gmtime(runingtime))
            )
        )
...
def woepoints_ply1(dtx, binx, x_i, woe_points):
    ...
    if is_numeric_dtype(dtx[x_i]):
        ...
        breaks_binx_other = np.unique(
            list(
                map(
                    float,
                    ["-inf"]
                    + [re.match(r".*\[(.*),.+\).*", str(i)).group(1) for i in binx_other["bin"]]
                    + ["inf"],
                )
            )
        )
        ...
        dtx = dtx.assign(
            xi_bin=lambda x: pd.cut(x[x_i], breaks_binx_other, right=False, labels=labels)
...
def plot_bin(binx, title, show_iv):
    ...
    # title
    title_string = (
        binx.loc[0, "variable"] + "  (iv:" + str(round(binx.loc[0, "total_iv"], 4)) + ")"
        if show_iv
    ...
    for i in ind:
        ax1.text(
            ...
            str(round(binx.loc[i, "count_distr"] * 100, 1)) + "%, " + str(binx.loc[i, "count"]),
            ha="center",
        )
    # ax2
...
def woebin_adj(
    ...
    if not adj_all_var:
        ...
        bins2["badprob2"] = (
            bins2.groupby("variable").apply(lambda x: x["badprob"].shift(1)).reset_index(drop=True)
        )
