...
def ie(dt, y="creditability", x=None, order=True):
    ...
    # info_ent
    ielist = pd.DataFrame(
        {"variable": x, "info_ent": [ie_xy(dt[i], dt[y]) for i in x]},
...
def ie_01(good, bad):
    ...
    df_enti = (
        ...
        .assign(enti=lambda x: -(x.p0.apply(enti) + x.p1.apply(enti)))
    )
    # xN_distr
...
def ig(dt, y, x=None, order=True):
    ...
    # info_ent
    iglist = pd.DataFrame(
        {"variable": x, "gini_impurity": [ig_xy(dt[i], dt[y]) for i in x]},
...
def ig_01(good, bad):
    ...
    df_gini = (
        ...
        .assign(bin_ig=lambda x: 1 - (x.p0**2 + x.p1**2))
    )
    # xN_distr
