...
def one_hot(
    dt, cols_skip=None, cols_encode=None, nacol_rm=False, replace_na=-1, category_to_integer=False
):
    """
    One Hot Encoding
    ------
    ...
    if cols_encode is None:
        cols_encode = char_cols = [
            i for i in list(dt) if not is_numeric_dtype(dt[i]) and dt[i].dtypes != "datetime64[ns]"
        ]
    else:
    ...
    if cols_encode is None or len(cols_encode) == 0:
        ...
        dt_new = pd.concat(
            [dt.drop(cols_encode, axis=1), temp_dt.drop(rm_cols_nan1, axis=1)], axis=1
        )
    ...
    if replace_na is not None:
        ...
        if cols_skip is not None:
            names_fillna = list(set(names_fillna) - set(cols_skip))
