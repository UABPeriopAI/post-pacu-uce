...
class MRIServiceBuilder:
    ...
    def prepare(self, plan, image_file_path,subject_id, runx, table_name):
        ...
        if plan == "adni-kim":
              self._instance = MRIPreproc.KIM_MRI_Preproc(image_file_path, subject_id, runx, table_name)
        elif plan == "AMP":
              self._instance = MRIPreproc.AMP_MRI_Preproc(image_file_path, subject_id, runx, table_name)
